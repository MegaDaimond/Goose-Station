...
public sealed class CustomGhostPrototype : IPrototype, IInheritingPrototype
{
    ...
    public bool CanUse(ICommonSession session, int sponsorTier = 0) => CanUse(session, out _, out _, sponsorTier);
    public bool CanUse(ICommonSession session, out string fullFailReason, int sponsorTier = 0) => CanUse(session, out fullFailReason, out _, sponsorTier);
    public bool CanUse(ICommonSession session, out string fullFailReason, out bool canSee, int sponsorTier = 0)
    {
        canSee = true;
        ...
        {
            if (!restriction.CanUse(session, out var failReason, sponsorTier, session.UserId.ToString()))
            {
                result = false;
                fullFailReason += $"\n{failReason}";
...
public abstract class CustomGhostRestriction
{
    ...
    public abstract bool CanUse(ICommonSession player, [NotNullWhen(false)] out string? failReason, int sponsorTier = 0, string uuid = "");
}